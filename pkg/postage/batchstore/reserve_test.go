...
func TestBatchStoreUnreserve(t *testing.T) {
	...
	bStore.SetUnreserve(func(batchID []byte, depth uint8) error {
		unreserved[string(batchID)] = depth
		return nil
	})
	...
	finalDepth := batchstore.DefaultDepth + 4
	for step := 0; depth < uint8(finalDepth); step++ {
		...
		fmt.Printf("\nPUT  NEW %d, block=%d,  price=%d, total=%d, advance=%d,  depth=%d, limit=%d  -  new  batch: depth=%d, value=%d (period=%d)\n", step, cs.Block, cs.Price.Uint64(), cs.Total.Uint64(), advance, depth, limit.Uint64(), b.Depth, value.Uint64(), period)
